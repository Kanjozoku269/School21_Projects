...
echo -e $color_key$color_backkey"SPACE_ROOT_FREE"$reset_color = $color_val$color_valback$space_root_free$reset_color
	...
	echo -e "Column 1 background = $key_background_color("$color_backkey"$print_color_key_back"$reset_color")"
        echo -e "Column 1 font color = $key_font_color("$color_key"$print_color_key"$reset_color")"
        echo -e "Column 2 background = $value_background_color("$color_valback"$print_color_value_back"$reset_color")"
        echo -e "Column 2 font color = $value_font_color("$color_val"$print_color_value"$reset_color")"
