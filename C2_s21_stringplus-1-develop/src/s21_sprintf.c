...
s_info *initialise(s_info *ints) {
  ...
  ints->exp = 0;
  ints->flag_Lf = 0;
  ints->full_buf[CHAR_PATTERN - 1] = 0;
  return (ints);
}
...
int s21_sprintf(char *str, const char *format, ...) {
  ...
  va_end(ints->args);
  s21_size_t end = s21_strlen(ints->full_buf);
  free(ints);
  return end;
}
...
int print_format(s_info *ints, const char *format, int temp) {
  ...
  if (format[temp] == '*') {
    if (!ints->point) temp = check_wparg(ints, format, temp);
  }
  if (format[temp] == '0' && !ints->point) ints->zero_padding = 1;
  if (format[temp] == 'n') {
    int c;
    c = (int)s21_strlen(ints->full_buf);
...
void print_unsigned(s_info *ints) {
  ...
  uns = va_arg(ints->args, unsigned long long int);
...
char *s_strcat(char *dest, const char *src) {
  ...
  for (s21_size_t i = start, j = 0; j <= end; i++, j++) {
    dest[i] = src[j];
    if (src[j] == '\0') {
      break;
    }
...
void s21_sprintf_g(s_info *ints) {
  ...
  if (pow <= 4) {
    ...
    s21_size_t number = s21_strlen(buff) - 1;
    while (number) {
      if (buff[number] == '0') {
        buff[number] = '\0';
        number--;
    ...
    if (!ints->sign && ints->space) {
      if (ints->width - s21_strlen(buff) > 1) {
        s_strcat(ints->full_buf, " ");
      }
    }
    if (ints->sign) {
...
void print_float(s_info *ints) {
  ...
  if (ints->flag_Lf) numb_fl = va_arg(ints->args, long double);
  ...
  len = s21_strlen(pattern_f_d);
  if (ints->width) ints->width -= len;
  if (ints->space || ints->sign) ints->width--;
...
void print_hex(s_info *ints) {
  ...
  hex = va_arg(ints->args, unsigned long long int);
  ints->hex_size = 0;
  if (ints->flag_o) {
...
void print_hexup(s_info *ints) {
  long long int hexup;
  hexup = va_arg(ints->args, long long int);
  ints->hex_size = 0;
...
int check_format_letter_2(const char *format, int temp, s_info *ints) {
  if ((format[temp] != 'c' && format[temp] != 's' && format[temp] != 'p' &&
    ...
    ints->star_width++;
  else if (format[temp] == '0' && ints->point)
    ints->flag_zero++;
  return 0;
}
...
void width_full(s_info *ints, int flag) {
  ...
  s21_size_t l = s21_strlen(ints->full_buf);
  if (!flag) {
    if (!ints->flag_f_float && ints->sign) {
      ints->width--;
      if (ints->space) {
        ints->width++;
        ints->space = 0;
      }
      for (s21_size_t i = l; i < l + ints->width; i++) {
...
void point_b(s_info *ints, unsigned long long int numb, char x) {
  char *base;
  long long int i = 0;
  char l[100] = {0};
  if (x == 'x') {
...
void point_o(s_info *ints, unsigned long long int numb, char o) {
  char *base;
...
int str_else(char *s, s_info *ints) {
  ...
  if (ints->flag_dash && !ints->space && !ints->sign) {
    s_strcat(ints->full_buf, "-");
    ints->j_save_format++;
  }
...
char *s_strncat(char *dest, const char *src, s21_size_t n) {
  ...
  s21_size_t end = s21_strlen(src);
  if (n > end) n = end;
  for (s21_size_t i = start, j = 0; j < n; i++, j++) {
    dest[i] = src[j];
  }
  return dest;
}
